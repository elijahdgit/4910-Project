...
die ('Please complete the registration form!<br><a href="admin_registration.html">Back</a>');
}

$rand = rand(1, 500000);

$tempStmt1 = 'SELECT adminID, adminPassword FROM Admin WHERE adminName = ?';
$tempStmt2 = 'INSERT INTO Admin (adminID, adminName, adminPassword, adminEmail, adminPhone, adminStatus) VALUES (?, ?, ?, ?, ?, ?)';
...
$stmt->bind_param('ssssss', $rand, $_POST['username'], $password, $_POST['email'], $_POST['phone'],
    $_POST['status']);
$stmt->execute();
printf("Error: %s.\n", $stmt->error);
